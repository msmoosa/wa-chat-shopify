<template>
    <s-page>
        ...
        <template v-else>
            <s-section v-if="showOnboardingGuide" heading="Get Started">
                <s-grid gap="base">
                    <s-grid gap="small-200">
                        <s-grid gridTemplateColumns="1fr auto" gap="small-300" alignItems="center">
                            <s-heading>Setup Guide</s-heading>
                            <s-button accessibilityLabel="Dismiss Guide" variant="tertiary" tone="neutral" icon="x"
                                @click="dismissOnboarding" />
                        </s-grid>
                        <s-paragraph>
                            Use this personalized guide to get your store ready.
                        </s-paragraph>
                    </s-grid>
                        ...
                        <s-box>
                            ...
                            <s-box padding="small" paddingBlockStart="none">
                                <s-button @click="navigateToWhatsappChatAppBlock" variant="secondary">Enable Whatsapp
                                    Chat App Block</s-button>
                            </s-box>
                        </s-box>
                        <s-divider />
                        <s-box>
                            <s-grid gridTemplateColumns="1fr auto" gap="base" padding="small">
                                <s-checkbox label="Configure Whatsapp Button Settings" />
                            </s-grid>
                            <s-box padding="small" paddingBlockStart="none">
                                <s-button @click="configureWhatsappButtonSettings" variant="secondary">Configure Button
                                    Settings</s-button>
                            </s-box>
                    ...
                    <s-table>
                        ...
                        <s-table-body>
                            <s-table-row v-for="agent in config.widgetAgents" :key="$index">
                                ...
                                <s-table-cell><s-select :value="agent.gender"
                                        @input="agent.gender = $event.target.value">
...
export default {
    ...
    methods: {
        ...
        async loadShopConfig() {
            try {
                ...
                if (result.success && result.data) {
                    ...
                    this.showOnboardingGuide = !(result.data.onboarding_completed ?? false);

                    if (result.data.whatsapp_config) {
                    ...
                    // Initialize with default config if API fails
                    this.config = this.getDefaultConfig();
                }
            } catch (error) {
        ...
        navigateToWhatsappChatAppBlock() {
            // open in new tab
        ...
        async completeOnboarding() {
            try {
                ...
                // Still hide the guide locally even if API call fails
                this.showOnboardingGuide = false;
            }
        },
        async dismissOnboarding() {
            // Mark onboarding as completed when user dismisses
            await this.completeOnboarding();
        },
        getDefaultConfig() {
            return {
                ...
                buttonTextContent: 'Chat with us',
                isEnabledOnDesktop: true,
                isEnabledOnMobile: true,
                buttonMarginDesktop: '20',
                buttonMarginMobile: '10',
                isDefaultMessageEnabled: false,
                defaultMessageText: '',
                isWidgetEnabled: false,
                widgetAgents: [],
                widgetHeaderTitle: 'Chat with us',
                widgetHeaderDescription: 'Select an agent to start chatting',
                widgetHeaderBackgroundColor: '#25D366',
                widgetHeaderSecondaryColor: '#128C7E',
                widgetHeaderTextColor: '#ffffff',
                iconGradientSecondColor: '#128C7E',
                buttonIconFlag: 'ind',
                buttonIconSize: '64'
            };
        }
